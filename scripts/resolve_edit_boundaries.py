...
LLM_DELETE_SOURCES = {"semantic_dedup", "content_cleanup", "local_false_start_refine"}
...
def resolve_edit_boundaries(
    ...
    delete_padding_before = float(cfg.get("semantic_delete_padding_before", 0.0))
    delete_padding_after = float(cfg.get("semantic_delete_padding_after", 0.0))
    ...
    for edit in edits:
        ...
        start, end = _snap_to_covered_word_boundaries(transcript, start, end)
        if edit.source in {"semantic_dedup", "local_false_start_refine"}:
            start, end = _apply_delete_padding(
                transcript,
                start,
                end,
                delete_padding_before,
                delete_padding_after,
            )
        if end <= start:
            reviews.append(
                {
...
def _snap_to_covered_word_boundaries(transcript: Transcript, start: float, end: float) -> tuple[float, float]:
    """Keep LLM delete boundaries on provider word edges after optional VAD alignment."""
    covered_words = _words_in_range(transcript, start, end)
    provider_words = [w for w in covered_words if w.timestamp_source == "provider"]
    if not provider_words:
        return start, end
    return min(w.start for w in provider_words), max(w.end for w in provider_words)


def _apply_delete_padding(
    transcript: Transcript,
    start: float,
    end: float,
    ...
    """Expand delete ranges slightly without eating neighboring provider words."""
    if padding_before <= 0 and padding_after <= 0:
        return start, end

    words = sorted(
        [word for seg in transcript.segments for word in seg.words if word.timestamp_source == "provider"],
    ...
    for word in words:
        if word.end <= start:
            prev_word_end = max(prev_word_end, word.end)
        elif word.start >= end:
            next_word_start = word.start
            break

    padded_start = max(prev_word_end, start - padding_before, 0.0)
    padded_end = end + padding_after
    if next_word_start is not None:
        padded_end = min(padded_end, next_word_start)
