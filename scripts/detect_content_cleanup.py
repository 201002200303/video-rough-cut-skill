...
def detect_content_cleanup(
    ...
    llm_review_enabled = bool(cfg.get("llm_review_enabled", review_on_uncertain))

    if not enabled or (not auto_delete_enabled and not llm_review_enabled):
        logger.info("Content cleanup LLM skipped: enabled=%s auto_delete=%s llm_review=%s", enabled, auto_delete_enabled, llm_review_enabled)
        return [], []
